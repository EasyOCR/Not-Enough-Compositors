...
`WAYLAND_DISPLAY` / `DISPLAY`) and hands back a `Box<dyn Compositor>` —
calling code never branches on which compositor it's talking to.
...
- **Hyprland**, via `hyprctl`'s own socket IPC rather than a Wayland
  ...
  positioning and sizing — something no other Wayland compositor exposes
...
Wayland path except Hyprland's own IPC, so callers can detect and handle
the gap rather than assume the call silently no-oped.
...
- **Wayland (Plasma and ext-foreign-toplevel-list paths) and Hyprland's
  IPC backend**: implemented against the published protocol/IPC specs,
  but not yet verified against a live KDE, GNOME, or Hyprland session —
  none were available in the environment this was built in (Hyprland in
  particular needs a GPU-backed wlroots build). Treat these as needing
  real-world confirmation before depending on them.
